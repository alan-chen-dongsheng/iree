// -----// IR Dump After Inliner (inline) //----- //
module {
  func.func @SingleConv(%arg0: !torch.vtensor<[1,3,500,500],f32>) -> !torch.vtensor<[1,8,498,498],f32> attributes {torch.assume_strict_symbolic_shapes, torch.onnx_meta.ir_version = 13 : si64, torch.onnx_meta.opset_version = 25 : si64, torch.onnx_meta.producer_name = "single_conv_example", torch.onnx_meta.producer_version = ""} {
    %cst = arith.constant dense<[0.540064871, 1.66891348, -0.204370424, 0.173146293, -0.0351918153, -0.198536336, 0.0749602392, 0.360360771]> : tensor<8xf32>
    %cst_0 = arith.constant dense<""> : tensor<8x3x3x3xf32>
    %0 = torch_c.to_builtin_tensor %arg0 : !torch.vtensor<[1,3,500,500],f32> -> tensor<1x3x500x500xf32>
    %1 = torch_c.from_builtin_tensor %cst_0 : tensor<8x3x3x3xf32> -> !torch.vtensor<[8,3,3,3],f32>
    %2 = torch_c.to_builtin_tensor %1 : !torch.vtensor<[8,3,3,3],f32> -> tensor<8x3x3x3xf32>
    %3 = torch_c.from_builtin_tensor %cst : tensor<8xf32> -> !torch.vtensor<[8],f32>
    %4 = torch_c.to_builtin_tensor %3 : !torch.vtensor<[8],f32> -> tensor<8xf32>
    %5 = tensor.empty() : tensor<1x8x498x498xf32>
    %broadcasted = linalg.broadcast ins(%4 : tensor<8xf32>) outs(%5 : tensor<1x8x498x498xf32>) dimensions = [0, 2, 3] 
    %6 = linalg.conv_2d_nchw_fchw {dilations = dense<1> : vector<2xi64>, strides = dense<1> : vector<2xi64>} ins(%0, %2 : tensor<1x3x500x500xf32>, tensor<8x3x3x3xf32>) outs(%broadcasted : tensor<1x8x498x498xf32>) -> tensor<1x8x498x498xf32>
    %7 = torch_c.from_builtin_tensor %6 : tensor<1x8x498x498xf32> -> !torch.vtensor<[1,8,498,498],f32>
    return %7 : !torch.vtensor<[1,8,498,498],f32>
  }
}


