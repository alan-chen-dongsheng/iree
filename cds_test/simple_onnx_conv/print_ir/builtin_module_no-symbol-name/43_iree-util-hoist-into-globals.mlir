...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  util.func public @SingleConv$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
    ...
    %2 = flow.dispatch.region -> (tensor<8x498x498xf32>) {
      ...
      %8 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "parallel", "reduction", "reduction", "reduction"]} ins(%collapsed, %cst : tensor<3x500x500xf32>, tensor<8x3x3x3xf32>) outs(%7 : tensor<8x498x498xf32>) {
      ^bb0(%in: f32, %in_2: f32, %out: f32):
        %10 = arith.mulf %in, %in_2 : f32
        %11 = arith.addf %out, %10 : f32
        linalg.yield %11 : f32
      } -> tensor<8x498x498xf32>
      %9 = linalg.generic {indexing_maps = [#map3, #map4, #map3], iterator_types = ["parallel", "parallel", "parallel"]} ins(%8, %cst_0 : tensor<8x498x498xf32>, tensor<8xf32>) outs(%6 : tensor<8x498x498xf32>) {
      ^bb0(%in: f32, %in_2: f32, %out: f32):
