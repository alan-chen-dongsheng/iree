...
util.func public @SingleConv$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
  ...
  %4 = linalg.fill ins(%cst : f32) outs(%2 : tensor<8x498x498xf32>) -> tensor<8x498x498xf32>
  %5 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3, d4, d5) -> (d3, d1 + d4, d2 + d5)>, affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d3, d4, d5)>, affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel", "reduction", "reduction", "reduction"]} ins(%collapsed, %cst_0 : tensor<3x500x500xf32>, tensor<8x3x3x3xf32>) outs(%4 : tensor<8x498x498xf32>) {
  ^bb0(%in: f32, %in_2: f32, %out: f32):
    %10 = arith.mulf %in, %in_2 : f32
    %11 = arith.addf %out, %10 : f32
    linalg.yield %11 : f32
  } -> tensor<8x498x498xf32>
  %6 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%5, %3 : tensor<8x498x498xf32>, tensor<8x498x498xf32>) outs(%4 : tensor<8x498x498xf32>) {
  ^bb0(%in: f32, %in_2: f32, %out: f32):
    %10 = arith.addf %in, %in_2 : f32
    linalg.yield %10 : f32
  } -> tensor<8x498x498xf32>
  %expanded = tensor.expand_shape %6 [[0, 1], [2], [3]] output_shape [1, 8, 498, 498] : tensor<8x498x498xf32> into tensor<1x8x498x498xf32>
  %7 = iree_tensor_ext.compute_barrier.end %expanded : tensor<1x8x498x498xf32> -> tensor<1x8x498x498xf32>
  %8 = hal.tensor.barrier join(%7 : tensor<1x8x498x498xf32>) => %arg2 : !hal.fence
  %9 = hal.tensor.export %8 : tensor<1x8x498x498xf32> -> !hal.buffer_view
