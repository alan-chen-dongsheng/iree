// -----// IR Dump After RemoveTensorBarriersPass (iree-dispatch-creation-remove-tensor-barriers) //----- //
util.func public @SingleConv$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
  %cst = arith.constant dense<""> : tensor<8x3x3x3xf32>
  %0 = hal.tensor.import wait(%arg1) => %arg0 : !hal.buffer_view -> tensor<1x3x500x500xf32>
  %collapsed = tensor.collapse_shape %0 [[0, 1], [2], [3]] : tensor<1x3x500x500xf32> into tensor<3x500x500xf32>
  %1 = flow.dispatch.region -> (tensor<8x498x498xf32>) {
    %4 = tensor.empty() : tensor<8x498x498xf32>
    %cst_0 = arith.constant dense<[0.540064871, 1.66891348, -0.204370424, 0.173146293, -0.0351918153, -0.198536336, 0.0749602392, 0.360360771]> : tensor<8xf32>
    %cst_1 = arith.constant 0.000000e+00 : f32
    %5 = linalg.fill ins(%cst_1 : f32) outs(%4 : tensor<8x498x498xf32>) -> tensor<8x498x498xf32>
    %6 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3, d4, d5) -> (d3, d1 + d4, d2 + d5)>, affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d3, d4, d5)>, affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel", "reduction", "reduction", "reduction"]} ins(%collapsed, %cst : tensor<3x500x500xf32>, tensor<8x3x3x3xf32>) outs(%5 : tensor<8x498x498xf32>) {
    ^bb0(%in: f32, %in_2: f32, %out: f32):
      %8 = arith.mulf %in, %in_2 : f32
      %9 = arith.addf %out, %8 : f32
      linalg.yield %9 : f32
    } -> tensor<8x498x498xf32>
    %7 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%6, %cst_0 : tensor<8x498x498xf32>, tensor<8xf32>) outs(%4 : tensor<8x498x498xf32>) {
    ^bb0(%in: f32, %in_2: f32, %out: f32):
      %8 = arith.addf %in, %in_2 : f32
      linalg.yield %8 : f32
    } -> tensor<8x498x498xf32>
    flow.return %7 : tensor<8x498x498xf32>
  }
  %expanded = tensor.expand_shape %1 [[0, 1], [2], [3]] output_shape [1, 8, 498, 498] : tensor<8x498x498xf32> into tensor<1x8x498x498xf32>
  %2 = hal.tensor.barrier join(%expanded : tensor<1x8x498x498xf32>) => %arg2 : !hal.fence
  %3 = hal.tensor.export %2 : tensor<1x8x498x498xf32> -> !hal.buffer_view
  util.return %3 : !hal.buffer_view
}

