...
util.func public @SingleConv$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
  ...
  %2 = flow.dispatch.region -> (tensor<8x498x498xf32>) {
    ...
    %8 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3, d4, d5) -> (d3, d1 + d4, d2 + d5)>, affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d3, d4, d5)>, affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel", "reduction", "reduction", "reduction"]} ins(%collapsed, %cst : tensor<3x500x500xf32>, tensor<8x3x3x3xf32>) outs(%7 : tensor<8x498x498xf32>) {
    ...
    flow.return %9 : tensor<8x498x498xf32>
  }
  %expanded = tensor.expand_shape %2 [[0, 1], [2], [3]] output_shape [1, 8, 498, 498] : tensor<8x498x498xf32> into tensor<1x8x498x498xf32>
  %3 = iree_tensor_ext.compute_barrier.end %expanded : tensor<1x8x498x498xf32> -> tensor<1x8x498x498xf32>
