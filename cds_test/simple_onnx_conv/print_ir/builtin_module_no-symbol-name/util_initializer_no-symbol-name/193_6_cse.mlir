...
^bb8:  // pred: ^bb7
  %executable = hal.executable.create device(%__device_0_4 : !hal.device) affinity(%c-1_i64) target(@SingleConv$async_dispatch_0::@embedded_elf_x86_64) : !hal.executable
...
^bb9:  // pred: ^bb7
  ...
  cf.br ^bb10(%1 : !hal.executable)
^bb10(%18: !hal.executable):  // 2 preds: ^bb8, ^bb9
  util.global.store %18, @__device_0_executable_0_SingleConv$async_dispatch_0 : !hal.executable
  %__device_0_5 = util.global.load immutable @__device_0 : !hal.device
  ...
  %allocator = hal.device.allocator<%__device_0_5 : !hal.device> : !hal.allocator
  ...
  cf.cond_br %did_import, ^bb12(%0, %mapped : !hal.fence, !hal.buffer), ^bb11
^bb11:  // pred: ^bb10
  ...
  %memory_file = hal.ex.file.from_memory device(%__device_0_5 : !hal.device) affinity(%c-1_i64) access(Read) buffer(%buffer_cst : !util.buffer)[%c0 for %c896] flags(%c0_i32) : !hal.file
  %fence = hal.fence.create device(%__device_0_5 : !hal.device) flags("None") : !hal.fence
  hal.device.queue.read<%__device_0_5 : !hal.device> affinity(%c-1_i64) wait(%0) signal(%fence) source(%memory_file : !hal.file)[%c0_i64] target(%buffer : !hal.buffer)[%c0] length(%c896) flags("None")
  cf.br ^bb12(%fence, %buffer : !hal.fence, !hal.buffer)
^bb12(%19: !hal.fence, %20: !hal.buffer):  // 2 preds: ^bb10, ^bb11
  %status = hal.fence.await until([%19]) timeout_millis(%c-1_i32) flags("None") : i32
  util.status.check_ok %status, "failed to wait on timepoint"
  util.global.store %20, @__constant_tensor_8x3x3x3xf32 : !hal.buffer
  %21 = util.call @__SingleConv$async_memoize_apply() : () -> !hal.command_buffer
  util.global.store %21, @__SingleConv$async_memoize_result_0_device_0 : !hal.command_buffer
  util.return
}
