// -----// IR Dump After LayoutSlicesPass (iree-stream-layout-slices) //----- //
util.initializer {
  %0 = stream.timepoint.immediate => !stream.timepoint
  %c864 = arith.constant 864 : index
  %1 = stream.timepoint.immediate => !stream.timepoint
  %buffer_cst = util.buffer.constant {alignment = 64 : index} : !util.buffer = #util.composite<896xi8, [
    dense<""> : tensor<8x3x3x3xf32>,
    dense<0> : vector<32xi8>,
]>
  %c896 = arith.constant 896 : index
  %c0 = arith.constant 0 : index
  %did_map, %result = stream.resource.try_map on(#hal.device.affinity<@__device_0>) %buffer_cst[%c0] : !util.buffer -> i1, !stream.resource<constant>{%c896}
  %c0_i64 = arith.constant 0 : i64
  %2:2 = scf.if %did_map -> (!stream.timepoint, !stream.resource<constant>) {
    scf.yield %1, %result : !stream.timepoint, !stream.resource<constant>
  } else {
    %7 = stream.resource.alloc uninitialized on(#hal.device.affinity<@__device_0>) : !stream.resource<constant>{%c896}
    %file = stream.file.constant on(#hal.device.affinity<@__device_0>) %buffer_cst[%c0 for %c896] : !util.buffer{%c896} -> !stream.file
    %8 = stream.file.read on(#hal.device.affinity<@__device_0>) await(%1) => %file[%c0_i64], %7[%c0], %c896 : !stream.file -> !stream.resource<constant>{%c896} => !stream.timepoint
    scf.yield %8, %7 : !stream.timepoint, !stream.resource<constant>
  }
  %3 = stream.resource.subview %2#1[%c0] : !stream.resource<constant>{%c896} -> !stream.resource<constant>{%c864}
  %4 = stream.cmd.execute once on(#hal.device.affinity<@__device_0>) with() {
  } => !stream.timepoint
  %5 = stream.timepoint.join max(%2#0, %4) => !stream.timepoint
  %6 = stream.timepoint.await sync %5 => %3 : !stream.resource<constant>{%c864}
  util.global.store %6, @__constant_tensor_8x3x3x3xf32 : !stream.resource<constant>
  util.global.store %0, @__constant_tensor_8x3x3x3xf32__timepoint : !stream.timepoint
  util.return
}

