// -----// IR Dump After Canonicalizer (canonicalize) //----- //
util.initializer {
  %c0_i64 = arith.constant 0 : i64
  %c0 = arith.constant 0 : index
  %c896 = arith.constant 896 : index
  %0 = stream.timepoint.immediate => !stream.timepoint
  %c864 = arith.constant 864 : index
  %buffer_cst = util.buffer.constant {alignment = 64 : index} : !util.buffer = #util.composite<896xi8, [
    dense<""> : tensor<8x3x3x3xf32>,
    dense<0> : vector<32xi8>,
]>
  %did_map, %result = stream.resource.try_map on(#hal.device.affinity<@__device_0>) %buffer_cst[%c0] : !util.buffer -> i1, !stream.resource<constant>{%c896}
  %1:2 = scf.if %did_map -> (!stream.timepoint, !stream.resource<constant>) {
    scf.yield %0, %result : !stream.timepoint, !stream.resource<constant>
  } else {
    %4 = stream.resource.alloc uninitialized on(#hal.device.affinity<@__device_0>) : !stream.resource<constant>{%c896}
    %file = stream.file.constant on(#hal.device.affinity<@__device_0>) %buffer_cst[%c0 for %c896] : !util.buffer{%c896} -> !stream.file
    %5 = stream.file.read on(#hal.device.affinity<@__device_0>) await(%0) => %file[%c0_i64], %4[%c0], %c896 : !stream.file -> !stream.resource<constant>{%c896} => !stream.timepoint
    scf.yield %5, %4 : !stream.timepoint, !stream.resource<constant>
  }
  %2 = stream.timepoint.await sync %1#0 => %1#1 : !stream.resource<constant>{%c896}
  %3 = stream.resource.subview %2[%c0] : !stream.resource<constant>{%c896} -> !stream.resource<constant>{%c864}
  util.global.store %3, @__constant_tensor_8x3x3x3xf32 : !stream.resource<constant>
  util.global.store %0, @__constant_tensor_8x3x3x3xf32__timepoint : !stream.timepoint
  util.return
}

