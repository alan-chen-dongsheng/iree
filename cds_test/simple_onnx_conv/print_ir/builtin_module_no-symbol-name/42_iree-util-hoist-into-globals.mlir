...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  util.func public @SingleConv$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
    ...
    %4 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "parallel", "reduction", "reduction", "reduction"]} ins(%collapsed, %cst_0 : tensor<3x500x500xf32>, tensor<8x3x3x3xf32>) outs(%3 : tensor<8x498x498xf32>) {
    ^bb0(%in: f32, %in_2: f32, %out: f32):
      %9 = arith.mulf %in, %in_2 : f32
      %10 = arith.addf %out, %9 : f32
      linalg.yield %10 : f32
    } -> tensor<8x498x498xf32>
    %5 = linalg.generic {indexing_maps = [#map3, #map4, #map3], iterator_types = ["parallel", "parallel", "parallel"]} ins(%4, %cst_1 : tensor<8x498x498xf32>, tensor<8xf32>) outs(%2 : tensor<8x498x498xf32>) {
    ^bb0(%in: f32, %in_2: f32, %out: f32):
      %9 = arith.addf %in, %in_2 : f32
      linalg.yield %9 : f32
    } -> tensor<8x498x498xf32>
    %expanded = tensor.expand_shape %5 [[0, 1], [2], [3]] output_shape [1, 8, 498, 498] : tensor<8x498x498xf32> into tensor<1x8x498x498xf32>
    %6 = iree_tensor_ext.compute_barrier.end %expanded : tensor<1x8x498x498xf32> -> tensor<1x8x498x498xf32>
    %7 = hal.tensor.barrier join(%6 : tensor<1x8x498x498xf32>) => %arg2 : !hal.fence
    %8 = hal.tensor.export %7 : tensor<1x8x498x498xf32> -> !hal.buffer_view
