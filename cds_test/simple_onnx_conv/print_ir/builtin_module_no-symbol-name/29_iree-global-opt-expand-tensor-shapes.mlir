...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  util.func public @SingleConv$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
    ...
    %broadcasted = linalg.broadcast ins(%cst_1 : tensor<8xf32>) outs(%1 : tensor<1x8x498x498xf32>) dimensions = [0, 2, 3] 
    %2 = tensor.empty() : tensor<1x8x498x498xf32>
    %3 = linalg.fill ins(%cst : f32) outs(%2 : tensor<1x8x498x498xf32>) -> tensor<1x8x498x498xf32>
    %4 = linalg.conv_2d_nchw_fchw {dilations = dense<1> : vector<2xi64>, strides = dense<1> : vector<2xi64>} ins(%0, %cst_0 : tensor<1x3x500x500xf32>, tensor<8x3x3x3xf32>) outs(%3 : tensor<1x8x498x498xf32>) -> tensor<1x8x498x498xf32>
    %5 = linalg.generic {indexing_maps = [#map, #map, #map], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%4, %broadcasted : tensor<1x8x498x498xf32>, tensor<1x8x498x498xf32>) outs(%3 : tensor<1x8x498x498xf32>) {
    ^bb0(%in: f32, %in_2: f32, %out: f32):
      %8 = arith.addf %in, %in_2 : f32
      linalg.yield %8 : f32
    } -> tensor<1x8x498x498xf32>
    %6 = hal.tensor.barrier join(%5 : tensor<1x8x498x498xf32>) => %arg2 : !hal.fence
    %7 = hal.tensor.export %6 : tensor<1x8x498x498xf32> -> !hal.buffer_view
