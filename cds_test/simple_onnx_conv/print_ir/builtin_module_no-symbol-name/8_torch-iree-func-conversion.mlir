// -----// IR Dump After FuncConversionPass (torch-iree-func-conversion) //----- //
module {
  util.func public @SingleConv$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
    %0 = hal.tensor.import wait(%arg1) => %arg0 : !hal.buffer_view -> tensor<1x3x500x500xf32>
    %1 = torch_c.from_builtin_tensor %0 : tensor<1x3x500x500xf32> -> !torch.vtensor<[1,3,500,500],f32>
    %cst = arith.constant dense<[0.540064871, 1.66891348, -0.204370424, 0.173146293, -0.0351918153, -0.198536336, 0.0749602392, 0.360360771]> : tensor<8xf32>
    %cst_0 = arith.constant dense<""> : tensor<8x3x3x3xf32>
    %2 = torch_c.to_builtin_tensor %1 : !torch.vtensor<[1,3,500,500],f32> -> tensor<1x3x500x500xf32>
    %3 = torch_c.from_builtin_tensor %cst_0 : tensor<8x3x3x3xf32> -> !torch.vtensor<[8,3,3,3],f32>
    %4 = torch_c.to_builtin_tensor %3 : !torch.vtensor<[8,3,3,3],f32> -> tensor<8x3x3x3xf32>
    %5 = torch_c.from_builtin_tensor %cst : tensor<8xf32> -> !torch.vtensor<[8],f32>
    %6 = torch_c.to_builtin_tensor %5 : !torch.vtensor<[8],f32> -> tensor<8xf32>
    %7 = tensor.empty() : tensor<1x8x498x498xf32>
    %broadcasted = linalg.broadcast ins(%6 : tensor<8xf32>) outs(%7 : tensor<1x8x498x498xf32>) dimensions = [0, 2, 3] 
    %8 = linalg.conv_2d_nchw_fchw {dilations = dense<1> : vector<2xi64>, strides = dense<1> : vector<2xi64>} ins(%2, %4 : tensor<1x3x500x500xf32>, tensor<8x3x3x3xf32>) outs(%broadcasted : tensor<1x8x498x498xf32>) -> tensor<1x8x498x498xf32>
    %9 = torch_c.from_builtin_tensor %8 : tensor<1x8x498x498xf32> -> !torch.vtensor<[1,8,498,498],f32>
    %10 = hal.tensor.barrier join(%8 : tensor<1x8x498x498xf32>) => %arg2 : !hal.fence
    %11 = hal.tensor.export %10 : tensor<1x8x498x498xf32> -> !hal.buffer_view
    util.return %11 : !hal.buffer_view
  }
  util.func public @SingleConv(%arg0: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub} {
    %c-1_i32 = arith.constant -1 : i32
    %c0 = arith.constant 0 : index
    %device_0 = hal.devices.get %c0 : !hal.device
    %0 = util.null : !hal.fence
    %fence = hal.fence.create device(%device_0 : !hal.device) flags("None") : !hal.fence
    %1 = util.call @SingleConv$async(%arg0, %0, %fence) : (!hal.buffer_view, !hal.fence, !hal.fence) -> !hal.buffer_view
    %status = hal.fence.await until([%fence]) timeout_millis(%c-1_i32) flags("None") : i32
    util.return %1 : !hal.buffer_view
  }
}


