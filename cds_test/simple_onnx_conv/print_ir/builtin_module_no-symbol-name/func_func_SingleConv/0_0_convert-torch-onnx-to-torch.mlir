// -----// IR Dump After ConvertTorchOnnxToTorch (convert-torch-onnx-to-torch) //----- //
func.func @SingleConv(%arg0: !torch.vtensor<[1,3,500,500],f32>) -> !torch.vtensor<[1,8,498,498],f32> attributes {torch.onnx_meta.ir_version = 13 : si64, torch.onnx_meta.opset_version = 25 : si64, torch.onnx_meta.producer_name = "single_conv_example", torch.onnx_meta.producer_version = ""} {
  %0 = torch.vtensor.literal(dense<""> : tensor<8x3x3x3xf32>) : !torch.vtensor<[8,3,3,3],f32>
  %1 = torch.vtensor.literal(dense<[0.540064871, 1.66891348, -0.204370424, 0.173146293, -0.0351918153, -0.198536336, 0.0749602392, 0.360360771]> : tensor<8xf32>) : !torch.vtensor<[8],f32>
  %none = torch.constant.none
  %int0 = torch.constant.int 0
  %int0_0 = torch.constant.int 0
  %int0_1 = torch.constant.int 0
  %int0_2 = torch.constant.int 0
  %int0_3 = torch.constant.int 0
  %2 = torch.prim.ListConstruct %int0, %int0_0 : (!torch.int, !torch.int) -> !torch.list<int>
  %int1 = torch.constant.int 1
  %int1_4 = torch.constant.int 1
  %int1_5 = torch.constant.int 1
  %int1_6 = torch.constant.int 1
  %3 = torch.prim.ListConstruct %int1, %int1_4 : (!torch.int, !torch.int) -> !torch.list<int>
  %4 = torch.prim.ListConstruct %int1_5, %int1_6 : (!torch.int, !torch.int) -> !torch.list<int>
  %5 = torch.prim.ListConstruct %int0_3, %int0_3 : (!torch.int, !torch.int) -> !torch.list<int>
  %false = torch.constant.bool false
  %int1_7 = torch.constant.int 1
  %6 = torch.aten.convolution %arg0, %0, %1, %4, %2, %3, %false, %5, %int1_7 : !torch.vtensor<[1,3,500,500],f32>, !torch.vtensor<[8,3,3,3],f32>, !torch.vtensor<[8],f32>, !torch.list<int>, !torch.list<int>, !torch.list<int>, !torch.bool, !torch.list<int>, !torch.int -> !torch.vtensor<[1,8,498,498],f32>
  return %6 : !torch.vtensor<[1,8,498,498],f32>
}

