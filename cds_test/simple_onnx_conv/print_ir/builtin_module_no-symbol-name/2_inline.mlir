...
module {
  func.func @SingleConv(%arg0: !torch.vtensor<[1,3,500,500],f32>) -> !torch.vtensor<[1,8,498,498],f32> attributes {torch.onnx_meta.ir_version = 13 : si64, torch.onnx_meta.opset_version = 25 : si64, torch.onnx_meta.producer_name = "single_conv_example", torch.onnx_meta.producer_version = ""} {
    ...
    %6 = torch.shape.calculate {
      ...
      %7 = torch.prim.ListConstruct %int1, %int3, %int500, %int500 : (!torch.int, !torch.int, !torch.int, !torch.int) -> !torch.list<int>
      %8 = torch.prim.ListConstruct %int8, %int3, %int3, %int3 : (!torch.int, !torch.int, !torch.int, !torch.int) -> !torch.list<int>
      %9 = torch.prim.ListConstruct  : () -> !torch.list<int>
      %10 = torch.aten.append.t %9, %int1 : !torch.list<int>, !torch.int -> !torch.list<int>
      %11 = torch.aten.append.t %9, %int8 : !torch.list<int>, !torch.int -> !torch.list<int>
