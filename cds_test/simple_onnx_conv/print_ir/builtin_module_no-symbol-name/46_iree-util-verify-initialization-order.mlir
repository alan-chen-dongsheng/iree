...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  util.func public @SingleConv$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
    ...
    %2 = flow.dispatch.workgroups(%1, %cst) : (tensor<3x500x500xf32>, tensor<8x3x3x3xf32>) -> tensor<8x498x498xf32> =
      ...
      %10 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "parallel", "reduction", "reduction", "reduction"]} ins(%6, %7 : tensor<3x500x500xf32>, tensor<8x3x3x3xf32>) outs(%9 : tensor<8x498x498xf32>) {
      ^bb0(%in: f32, %in_2: f32, %out: f32):
        %12 = arith.mulf %in, %in_2 : f32
        %13 = arith.addf %out, %12 : f32
        linalg.yield %13 : f32
      } -> tensor<8x498x498xf32>
      %11 = linalg.generic {indexing_maps = [#map3, #map4, #map3], iterator_types = ["parallel", "parallel", "parallel"]} ins(%10, %cst_1 : tensor<8x498x498xf32>, tensor<8xf32>) outs(%8 : tensor<8x498x498xf32>) {
      ^bb0(%in: f32, %in_2: f32, %out: f32):
