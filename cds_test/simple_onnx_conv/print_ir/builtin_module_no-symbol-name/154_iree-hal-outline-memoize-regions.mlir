...
module {
  ...
  ^bb1:  // pred: ^bb0
    ...
    %allocator_3 = hal.device.allocator<%__device_0_1 : !hal.device> : !hal.allocator
    %buffer = hal.allocator.allocate<%allocator_3 : !hal.allocator> affinity(%c-1_i64_2) type(%memory_types) usage(%buffer_usage_0) : !hal.buffer{%c896}
    %__device_0_4 = util.global.load immutable @__device_0 : !hal.device
    %c-1_i64_5 = arith.constant -1 : i64
    %c0_i32 = arith.constant 0 : i32
    %memory_file = hal.ex.file.from_memory device(%__device_0_4 : !hal.device) affinity(%c-1_i64_5) access(Read) buffer(%buffer_cst : !util.buffer)[%c0 for %c896] flags(%c0_i32) : !hal.file
    ...
    hal.device.queue.read<%__device_0_6 : !hal.device> affinity(%c-1_i64_7) wait(%0) signal(%fence) source(%memory_file : !hal.file)[%c0_i64] target(%buffer : !hal.buffer)[%c0] length(%c896) flags("None")
  ...
  ^bb1:  // pred: ^bb0
    ...
    %exe = hal.executable.lookup device(%0 : !hal.device) executable(@SingleConv$async_dispatch_0) : !hal.executable
    ...
    hal.command_buffer.dispatch<%cmd : !hal.command_buffer> target(%exe : !hal.executable)[%ordinal] workgroups([%c6889, %c1_1, %c1_2]) bindings([
      (%c0 : index)[%c0_0, %c3000000], 
      (%arg2 : !hal.buffer)[%c0_0, %c896], 
      (%c1 : index)[%c0_0, %c7936128]
    ]) flags("None")
  ...
  util.func public @SingleConv$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
    ...
    %0 = util.call @__SingleConv$async_memoize_lookup(%__device_0_1, %c-1_i64_2) : (!hal.device, i64) -> !hal.command_buffer
    hal.device.queue.execute.indirect<%__device_0_1 : !hal.device> affinity(%c-1_i64_2) wait(%fence) signal(%arg2) commands(%0) bindings([
      (%buffer : !hal.buffer)[%c0_3, %c3000000], 
      (%transient_buffer : !hal.buffer)[%c0_3, %c7936128]
    ]) flags("None")
    ...
    %c498_8 = arith.constant 498 : index
    %c0_9 = arith.constant 0 : index
    %view = hal.buffer_view.create buffer(%transient_buffer : !hal.buffer)[%c0_9, %c7936128] shape([%c1_7, %c8, %c498, %c498_8]) type(%element_type_f32_6) encoding(%dense_row_major_5) : !hal.buffer_view
