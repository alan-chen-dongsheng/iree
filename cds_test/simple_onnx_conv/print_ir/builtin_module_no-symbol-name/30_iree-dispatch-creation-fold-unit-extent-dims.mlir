...
#map2 = affine_map<(d0, d1, d2, d3, d4, d5) -> (d3, d1 + d4, d2 + d5)>
...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  util.func public @SingleConv$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
    ...
    %3 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel", "parallel"]} ins(%cst_1 : tensor<8xf32>) outs(%2 : tensor<8x498x498xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    ...
    %6 = linalg.generic {indexing_maps = [#map2, #map3, #map4], iterator_types = ["parallel", "parallel", "parallel", "reduction", "reduction", "reduction"]} ins(%collapsed, %cst_0 : tensor<3x500x500xf32>, tensor<8x3x3x3xf32>) outs(%5 : tensor<8x498x498xf32>) {
    ...
    %9 = linalg.generic {indexing_maps = [#map1, #map1, #map1], iterator_types = ["parallel", "parallel", "parallel"]} ins(%6, %3 : tensor<8x498x498xf32>, tensor<8x498x498xf32>) outs(%8 : tensor<8x498x498xf32>) {
    ^bb0(%in: f32, %in_2: f32, %out: f32):
