...
module attributes {iree.fixedpoint.iteration = 0 : index, iree.fixedpoint.modified, stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  util.func public @SingleConv$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
    ...
    %3 = stream.timepoint.join max(%2, %0) => !stream.timepoint
    ...
    %5 = stream.timepoint.join max(%4, %0, %result_timepoint) => !stream.timepoint
    %6 = stream.cmd.execute on(#hal.device.affinity<@__device_0>) await(%5) => with(%1 as %arg3: !stream.resource<external>{%c3000000}, %__constant_tensor_8x3x3x3xf32 as %arg4: !stream.resource<constant>{%c896}, %result as %arg5: !stream.resource<external>{%c7936128}) {
    ...
    stream.timepoint.chain_external on(#hal.device.affinity<@__device_0>) %6 => (%arg2 : !hal.fence)
    %7 = stream.tensor.export on(#hal.device.affinity<@__device_0>) %result : tensor<1x8x498x498xf32> in !stream.resource<external>{%c7936128} -> !hal.buffer_view
