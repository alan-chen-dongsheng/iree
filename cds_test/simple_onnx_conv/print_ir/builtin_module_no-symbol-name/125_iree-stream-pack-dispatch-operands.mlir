...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  stream.executable private @SingleConv$async_dispatch_0 {
    ...
    builtin.module {
      func.func @SingleConv$async_dispatch_0_conv_8x498x498x3x3x3_f32(%arg0: !stream.binding {stream.alignment = 64 : index}, %arg1: !stream.binding {stream.alignment = 64 : index}, %arg2: !stream.binding {stream.alignment = 64 : index}, %arg3: i32, %arg4: i32, %arg5: i32, %arg6: i32, %arg7: i32, %arg8: i32) {
        ...
        %c0 = arith.constant 0 : index
        %16 = stream.binding.subspan %arg0[%15#0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<3x500x500xf32>>
        %17 = stream.binding.subspan %arg1[%15#1] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<8x3x3x3xf32>>
        %18 = stream.binding.subspan %arg2[%15#2] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<8x498x498xf32>>
        %19 = iree_tensor_ext.dispatch.tensor.load %16, offsets = [0, 0, 0], sizes = [3, 500, 500], strides = [1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<3x500x500xf32>> -> tensor<3x500x500xf32>
        %20 = iree_tensor_ext.dispatch.tensor.load %17, offsets = [0, 0, 0, 0], sizes = [8, 3, 3, 3], strides = [1, 1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<8x3x3x3xf32>> -> tensor<8x3x3x3xf32>
        %21 = tensor.empty() : tensor<8x498x498xf32>
        %22 = linalg.fill ins(%cst_2 : f32) outs(%21 : tensor<8x498x498xf32>) -> tensor<8x498x498xf32>
        %23 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "parallel", "reduction", "reduction", "reduction"]} ins(%19, %20 : tensor<3x500x500xf32>, tensor<8x3x3x3xf32>) outs(%22 : tensor<8x498x498xf32>) {
        ^bb0(%in: f32, %in_3: f32, %out: f32):
          %25 = arith.mulf %in, %in_3 : f32
          %26 = arith.addf %out, %25 : f32
          linalg.yield %26 : f32
        } -> tensor<8x498x498xf32>
        %24 = linalg.generic {indexing_maps = [#map3, #map4, #map3], iterator_types = ["parallel", "parallel", "parallel"]} ins(%23, %cst : tensor<8x498x498xf32>, tensor<8xf32>) outs(%21 : tensor<8x498x498xf32>) {
        ^bb0(%in: f32, %in_3: f32, %out: f32):
          %25 = arith.addf %in, %in_3 : f32
          linalg.yield %25 : f32
        } -> tensor<8x498x498xf32>
        iree_tensor_ext.dispatch.tensor.store %24, %18, offsets = [0, 0, 0], sizes = [8, 498, 498], strides = [1, 1, 1] : tensor<8x498x498xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<8x498x498xf32>>
        return
      }
    }
  }
  util.func public @SingleConv$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
    ...
    %2 = stream.cmd.execute on(#hal.device.affinity<@__device_0>) await(%result_timepoint) => with(%0 as %arg3: !stream.resource<external>{%c3000000}, %__constant_tensor_8x3x3x3xf32 as %arg4: !stream.resource<constant>{%c896}, %result as %arg5: !stream.resource<external>{%c7936128}) {
      stream.cmd.dispatch @SingleConv$async_dispatch_0::@SingleConv$async_dispatch_0_conv_8x498x498x3x3x3_f32(%c0_i32, %c0_i32_2, %c0_i32_4, %c0_i32_7, %c0_i32_9, %c0_i32_12 : i32, i32, i32, i32, i32, i32) {
        ro %arg3[%c0_0 for %c3000000] : !stream.resource<external>{%c3000000},
        ro %arg4[%c0_0 for %c896] : !stream.resource<constant>{%c896},
