...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  stream.executable private @SingleConv$async_dispatch_0 {
    ...
    builtin.module {
      func.func @SingleConv$async_dispatch_0_conv_8x498x498x3x3x3_f32(%arg0: !stream.binding {stream.alignment = 64 : index}, %arg1: !stream.binding {stream.alignment = 64 : index}, %arg2: !stream.binding {stream.alignment = 64 : index}, %arg3: index {stream.values = [0 : index]}, %arg4: index {stream.values = [0 : index]}, %arg5: index {stream.values = [0 : index]}) {
        %0:3 = util.assume.int 
            %arg3<umin = 0, umax = 0>, 
            %arg4<umin = 0, umax = 0>, 
            %arg5<umin = 0, umax = 0>
        ...
        %c0 = arith.constant 0 : index
        %1 = stream.binding.subspan %arg0[%0#0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<3x500x500xf32>>
        %2 = stream.binding.subspan %arg1[%0#1] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<8x3x3x3xf32>>
        %3 = stream.binding.subspan %arg2[%0#2] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<8x498x498xf32>>
        %4 = iree_tensor_ext.dispatch.tensor.load %1, offsets = [0, 0, 0], sizes = [3, 500, 500], strides = [1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<3x500x500xf32>> -> tensor<3x500x500xf32>
        %5 = iree_tensor_ext.dispatch.tensor.load %2, offsets = [0, 0, 0, 0], sizes = [8, 3, 3, 3], strides = [1, 1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<8x3x3x3xf32>> -> tensor<8x3x3x3xf32>
        %6 = tensor.empty() : tensor<8x498x498xf32>
        %7 = linalg.fill ins(%cst_0 : f32) outs(%6 : tensor<8x498x498xf32>) -> tensor<8x498x498xf32>
        %8 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "parallel", "reduction", "reduction", "reduction"]} ins(%4, %5 : tensor<3x500x500xf32>, tensor<8x3x3x3xf32>) outs(%7 : tensor<8x498x498xf32>) {
        ^bb0(%in: f32, %in_1: f32, %out: f32):
          %10 = arith.mulf %in, %in_1 : f32
          %11 = arith.addf %out, %10 : f32
          linalg.yield %11 : f32
        } -> tensor<8x498x498xf32>
        %9 = linalg.generic {indexing_maps = [#map3, #map4, #map3], iterator_types = ["parallel", "parallel", "parallel"]} ins(%8, %cst : tensor<8x498x498xf32>, tensor<8xf32>) outs(%6 : tensor<8x498x498xf32>) {
        ^bb0(%in: f32, %in_1: f32, %out: f32):
          %10 = arith.addf %in, %in_1 : f32
          linalg.yield %10 : f32
        } -> tensor<8x498x498xf32>
        iree_tensor_ext.dispatch.tensor.store %9, %3, offsets = [0, 0, 0], sizes = [8, 498, 498], strides = [1, 1, 1] : tensor<8x498x498xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<8x498x498xf32>>
        return
      }
    }
  }
  util.func public @SingleConv$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
    ...
    %2 = stream.cmd.execute on(#hal.device.affinity<@__device_0>) await(%result_timepoint) => with(%0 as %arg3: !stream.resource<external>{%c3000000}, %__constant_tensor_8x3x3x3xf32 as %arg4: !stream.resource<constant>{%c896}, %result as %arg5: !stream.resource<external>{%c7936128}) {
      stream.cmd.dispatch @SingleConv$async_dispatch_0::@SingleConv$async_dispatch_0_conv_8x498x498x3x3x3_f32(%c0, %c0, %c0 : index, index, index) {
